...
`(^\w+)@(\w+)\.(com|ca|gov)$`
```
This is Regex logic to match an email, this says it must start with `words (^\w+)` and be continued with `@` followed by more `words (\w+)` this is usually the email provider, it is then seperated with a dot `\.` and then it specifies the domains allowed, `(com|ca|gov)` com or ca or gov then finally use the `$` to make sure it `ends with` this domain.
...
`^Hello`&emsp;&emsp; - matches any string that starts with "`H`ello"<br>
`World$`&emsp;&emsp; - matches any string that ends with "Worl`d`"<br>
`^Goodbye$`&emsp;- matches anything between `^$` "`Goodbye`"<br>
`or` &emsp;&emsp;&emsp;&emsp;- matches any strings that are the same entered "W`or`ld"
...
* `*` - will match a string followed by zero or more of the last character<br>
* `+` - will match a string followed by one or more other the last character<br>
* `?` - will match a string followed by zero or one the last character<br>
* `{}` - will match a string followed by any amount in the braces of the last character<br>
* `()*` - will match a string with any characters followed by zero or more copies of the string within the parentheses

Examples: 

`wasd*`- matches a string that has `'was'` followed by zero or more `'d'`<br>
`wasd+` - matches a string that has `'was'` followed by one or more `'d'`<br>
`wasd?` - matches a string that has `'was'` followed by zero or one `'d'`<br>
`wasd{3}` - matches a string that has `'was'` followed by 3 `'d'`<br>
`wasd{3,}` - matches a string that has `'was'` followed by 3 or more `'d'`<br>
`wasd{3,10}` - matches a string that has `'was'` followed by 3-10 `'d'`<br>
`wa(sd)*` - matches a string that has `'wa'` followed by zero or more copies of the pattern `'sd'`<br>
...
* `|` - the vertical line will match a string on the left or right of it<br>
...
* `\w` - matches a word character (any alphanumeric character including the underscore)<br>
* `\s` - matches a whitespace character (including tabs and line breaks)<br>
...
* g - Global, does not return after the first match. Which will restart any following searches from the end of the previous match. (Makes the expression search for all occurences)<br>
* m - Multi-line, When enabled the `Anchors(^$)` will match the start and end of a line, rather than the whole string.<br>
...
* `()` - parentheses creates a capture group.<br>
* `(?:)` - ?: disables the capturing group.<br>
...
* `[]` - matching any single character within the brackets.<br>
* `[]%` - matching the string inside the brackets before the `%`.<br>
...
* Between two characters in the same string, where one is a word character and the other is NOT<br>
...
* `\b` - A position that binds a word, or where a word starts or ends, it denotes a place between a word and a non-word character, at the start and end of a string.<br>
* `\B` - The **opposite** of a work boundary, the negation of `\b` an=bd matches any position a word boundary **doesn't**<br>
